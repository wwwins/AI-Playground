...
from diffusers import KandinskyPipeline, KandinskyPriorPipeline, KandinskyV22Pipeline, KandinskyV22PriorPipeline
...
pipe_prior = KandinskyV22PriorPipeline.from_pretrained(model_prior, torch_dtype=MODEL_DTYPE, low_cpu_mem_usage=LOW_MEMORY)
#pipe_prior.to("cpu")
pipe_prior.enable_model_cpu_offload()

pipe = KandinskyV22Pipeline.from_pretrained(model, torch_dtype=MODEL_DTYPE, low_cpu_mem_usage=LOW_MEMORY)
#pipe.to("cuda")
...
def mixing_images(text_prompt, text_negative_prompt, text_weight, img1_weight, img2_weight, img3_weight, guidance, steps, seed, img1, img2, img3):
    print("===============\nweight:{} {} {} {}\nguidance:{}\nsteps:{}\nseed:{}\n===============\n".format(text_weight, img1_weight, img2_weight, img3_weight, guidance, steps, seed))
...
def text2img():
    image_emb, negative_image_emb = pipe_prior(prompt, negative_prompt, guidance_scale=1.0).to_tuple()
    out = pipe(
            prompt,
            image_embeds = image_emb,
...
def demo():
    gr.Interface(fn=mixing_images, inputs=[
    	gr.Textbox(value=prompt),
    	gr.Textbox(value=negative_prompt),
        gr.Slider(0, 1, value=0, step=0.1, label="weight of prompt", info="Choose between 0 and 1"),
        gr.Slider(0, 1, value=0.5, step=0.1, label="weight of img1", info="Choose between 0 and 1"),
        gr.Slider(0, 1, value=0.5, step=0.1, label="weight of img2", info="Choose between 0 and 1"),
        gr.Slider(0, 1, value=0.2, step=0.1, label="weight of img3", info="Choose between 0 and 1"),
        gr.Slider(1, 20, value=7, step=0.1, label="guidance scale", info="Choose between 1 and 20"),
        gr.Slider(1, 200, value=50, step=1, label="number inference steps", info="Choose between 1 and 200"),
        gr.Number(value=-1),
        gr.Image(type="pil"),
        gr.Image(type="pil"),
        gr.Image(type="pil"),
    ], outputs=[
    	gr.Image(label="mixing image"),
